  ...
  public CommandLineRunner initData(
    ...
    return args -> {
      userInfoRepository.saveAll(
          List.of(
              UserInfo.builder().userId(1L).userName("kyle").email("user@example.com").passwd(encoder.encode("password")).role("USER").isActivate(true).build(),
              UserInfo.builder().userId(2L).userName("jhone").email("jhone@example.com").passwd(encoder.encode("password")).role("GUEST").isActivate(true).build(),
              UserInfo.builder().userId(3L).userName("lizzy").email("lizzy@example.com").passwd(encoder.encode("password")).role("USER").isActivate(true).build(),
              UserInfo.builder().userId(4L).userName("tom").email("tom@example.com").passwd(encoder.encode("password")).role("USER").isActivate(true).build()
          )
      );
